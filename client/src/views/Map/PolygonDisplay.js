...
const PolygonDisplay = (loading, features, map) =>{
      ...
      map.loadImage(
        ...
        function (error, image) {
          ...
          map.addLayer({
            ...
            'source': unique[index] ,
            'paint': {
              'fill-color': `rgb(${Math.floor(Math.random() * (254 - 1 + 1) ) + 1},${Math.floor(Math.random() * (254 - 1 + 1) ) + 1},${Math.floor(Math.random() * (254 - 1 + 1) ) + 1})`,
              'fill-opacity': 0.8
            }
            // 'layout': {
